...
parser.add_argument(
    ...
    default=4,
    type=int,
    help="mini-batch size (default: 4), this is the total " "batch size of all GPUs",
)
parser.add_argument(
...
parser.add_argument(
    "--latent-dim", default=1024, type=int, help="size of the input latent variable"
)
parser.add_argument(
    "--g-iter", default=1, type=int, help="number of generator pass per iteration"
)
parser.add_argument(
    "--lr-g", default=0.0001, type=float, help="learning rate for the generator"
)
parser.add_argument(
    "--lr-d", default=0.0004, type=float, help="learning rate for the discriminator"
)
parser.add_argument(
    "--lr-e", default=0.0001, type=float, help="learning rate for the encoder"
)
parser.add_argument("--data-dir", type=str, help="path to the preprocessed data folder")
parser.add_argument(
    "--exp-name", default="HA_GAN_run1", type=str, help="name of the experiment"
)
parser.add_argument(
...
def main():
    ...
    trainset = Volume_Dataset(
        data_dir=args.data_dir, fold=args.fold, num_class=args.num_class
    )
    ...
    if args.img_size == 256:
        from models.Model_HA_GAN_256 import (
            Discriminator,
            Generator,
            Encoder,
            Sub_Encoder,
        ...
        from models.Model_HA_GAN_128 import (
            Discriminator,
            Generator,
            Encoder,
            Sub_Encoder,
    ...
    G = Generator(
        mode="train", latent_dim=args.latent_dim, num_class=args.num_class
    ).cuda()
    ...
    sub_e_optimizer = optim.Adam(
        Sub_E.parameters(), lr=args.lr_e, betas=(0.0, 0.999), eps=1e-8
    )
    ...
    for iteration in range(args.continue_iter, args.num_iter):
        ...
        real_images_crop = real_images[
            :, :, crop_idx : crop_idx + args.img_size // 8, :, :
        ]
        ...
        if args.num_class == 0:  # unconditional
            ...
            fake_images, fake_images_small = G(
            ...
            d_real_loss = loss_f(y_real_pred, real_labels) + F.cross_entropy(
                y_real_class, class_label
            )

            # random generation
            noise = torch.randn((args.batch_size, args.latent_dim)).cuda()
            fake_images, fake_images_small = G(
                noise, crop_idx=crop_idx, class_label=class_label_onehot
            )
        ...
        with torch.no_grad():
            ...
            for crop_idx_i in range(0, args.img_size, args.img_size // 8):
                real_images_crop_i = real_images[
                    :, :, crop_idx_i : crop_idx_i + args.img_size // 8, :, :
                ]
        ...
        if args.num_class == 0:  # unconditional
            ...
            sub_x_hat_rec, sub_x_hat_rec_small = G(
                sub_z_hat, crop_idx=crop_idx, class_label=class_label_onehot
            )

        sub_e_loss = (
            loss_mse(sub_x_hat_rec, real_images_crop)
            + loss_mse(sub_x_hat_rec_small, real_images_small)
        ) / 2.0
        ...
        if iteration % 200 == 0:
            print(
                "[{}/{}]".format(iteration, args.num_iter),
        ...
        if iteration > 30000 and (iteration + 1) % 500 == 0:
            torch.save(
                {"model": G.state_dict(), "optimizer": g_optimizer.state_dict()},
                "./checkpoint/"
                + args.exp_name
                + "/G_iter"
                + str(iteration + 1)
            ...
            torch.save(
                {"model": D.state_dict(), "optimizer": d_optimizer.state_dict()},
                "./checkpoint/"
                + args.exp_name
                + "/D_iter"
                + str(iteration + 1)
            ...
            torch.save(
                {"model": E.state_dict(), "optimizer": e_optimizer.state_dict()},
                "./checkpoint/"
                + args.exp_name
                + "/E_iter"
                + str(iteration + 1)
