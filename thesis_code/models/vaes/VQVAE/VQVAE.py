...
from .evaluate import nmse, psnr, SSIM3DSlices
...
def _eval_metrics_log_dict(orig, pred):
    ...
    return {func_name: func(orig, pred) for func_name, func in metrics}
...
class VQVAE(nn.Module):
    ...
    def __init__(self, args: Namespace):
        ...
        def init_fixupresblock(layer):
            if isinstance(layer, FixupResBlock) or isinstance(
                layer, PreActFixupResBlock
            ):
    ...
    def huber(
        self, batch, batch_idx, **pre_loss_f_metrics
    ...
    def _parse_input_args(self, args: Namespace):
        ...
        if len(args.num_embeddings) == 1:
            self.num_embeddings = [
                args.num_embeddings[0] for _ in range(args.n_bottleneck_blocks)
            ]
        else:
            self.num_embeddings = args.num_embeddings

        resblocks = {
            "regular": FixupResBlock,
            "pre-activation": PreActFixupResBlock,
            "evonorm": EvonormResBlock,
        }
        ...
        self.pre_loss_f = (
            None  # ExtractCenterCylinder() if args.extract_center_cylinder else None
        )
    ...
    def add_model_specific_args(cls, parent_parser):
        ...
        parser.add_argument(
            ...
            nargs="+",
            help=(
                "Can be either a single int or multiple."
        ...
        parser.add_argument(
            ...
            choices=["regular", "pre-activation", "evonorm"],
        )
        ...
        parser.add_argument(
            "--metric", choices=cls.supported_metrics, default=cls.supported_metrics[0]
        )
