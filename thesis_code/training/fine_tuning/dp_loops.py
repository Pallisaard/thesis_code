...
def dp_validation_step(
    ...
    val_total_loss_metric = val_g_loss_metric + val_d_loss_metric + val_e_loss_metric + val_sub_e_loss_metric
...
def training_loop_until_epsilon(
    ...
    with tqdm(desc="DP training progress.", dynamic_ncols=True, leave=True) as pbar:
        while state.training_stats.current_epsilon < max_epsilon:
            ...
            if max_steps is not None and state.training_stats.step >= max_steps:
                ...
                checkpoint_name = f"dp_model_epsilon={state.training_stats.current_epsilon:.2f}_steps={state.training_stats.step}_max_steps_reached.pth"
    ...
    checkpoint_name = (
        f"dp_model_epsilon={state.training_stats.current_epsilon:.2f}_steps={state.training_stats.step}.pth"
    )
    checkpoint_path_full = Path(checkpoint_path) / checkpoint_name
    print(f"Saving final checkpoint: {checkpoint_path_full}")
    checkpoint_dp_model(models, state, str(checkpoint_path_full))
